...
| bulletLifetime    | 子弹可存活时间[^10] |
| bulletTime        | 子弹已存活时间[^10] |
| enabled           | 是否启用            |
...
[^8]: 从核心获取, 返回核心所属队伍的最大同种单位数,
      如果是波次队伍等无上限的情况则返回一个巨大的数字

[^9]: 在高版本(约>157.4)添加的功能, 多人游戏时可以标记一个地点并留下备注,
      例如移动端为三击某个位置, 会产生一个无备注的标记点.
      如果在聊天框发送如 `2,3 test` 会在坐标 `2,3` 处创建一个备注为 `test` 的标记点

[^10]: 用于特权语句(世处) bullet 产生的返回值, 可以跟踪子弹情况
