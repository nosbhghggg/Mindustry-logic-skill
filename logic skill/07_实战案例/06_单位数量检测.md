...
### 解说
借助 lookup 指令，可以遍历所有友方单位类型，得到一个能打印在场全部单位数量的逻辑。注意：只能检查友方单位。

#### 工作流程
1. `lookup unit unitType idUnit`：根据 ID 获取单位类型（content 类型变量）
2. `op add idUnit idUnit 1`：ID 递增
3. `jump 5 lessThan idUnit @unitCount`：如果 ID < 友方单位类型总数，继续遍历下一种单位类型
...
#### 只能检查友方单位
`@unitCount` 只统计友方单位类型。敌方单位无法通过此方式检测，因为 `lookup unit` 遍历的是友方单位类型列表。
...
- `@unitCount` 环境变量表示友方单位类型总数
- `lookup unit` + `ubind unitType` 可以遍历所有友方单位类型逐一计数
