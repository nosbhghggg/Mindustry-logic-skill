...
| order | 整数 | 排序方向（1 = 正序/降序，0/负值 = 反序/升序） |
...
**缓存机制**：为防止雷达指令频繁执行导致的性能问题，雷达结果会被缓存。建筑类雷达每 30 tick 更新一次结果，在此期间返回上次缓存的目标。当搜索源建筑变化时也会触发重新搜索。
...
- 获取搜索范围 `r.range()`。
- **缓存更新**：建筑类搜索源每 30 tick 更新一次，或当搜索源建筑变化时更新。单位类搜索源使用每控制器实例的独立计时器。
- **搜索逻辑**：
...
4. 雷达结果有 30 tick 的缓存间隔，不会每次执行都重新搜索，这是为了性能优化。
5. `order` 参数为 1 时取排序最大值（如 distance 最远、health 最高），为 0 或负值时取最小值（如 distance 最近、health 最低）。
...
- 缓存机制（30 tick 间隔）在客户端和服务端独立运行，可能导致短暂的结果不一致。
